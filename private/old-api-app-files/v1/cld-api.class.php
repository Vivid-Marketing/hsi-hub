    ...
    public function downloadRemoteSrcToLocal($remoteSrc, $cldid, $lessonName)
    {
        ...
        if (! empty($remoteSrc)) {
            ...
            if (! parent::ensureDirectoryExists($target_dir)) {
                error_log("Failed to create or access directory: {$target_dir}");

                return false;
            }

            // echo $cldid . PHP_EOL;
            $imageContent = @file_get_contents($remoteSrc);
            if ($imageContent === false) {
                error_log("Failed to download remote image: {$remoteSrc}");

                return false;
            }

            if (@file_put_contents($img, $imageContent) === false) {
                error_log("Failed to write image file: {$img}");
